...
<script>
	var Ajax;
	function ajaxSendGoCode() {
		if(window.event.keyCode == 13) {
			var goodsCode = 
				encodeURIComponent(
						document.getElementsByName("goodsCode")[0].value);
			
			var orderCode = null;
			if(document.getElementsByName("orderCode")[0]){ 
				orderCode = encodeURIComponent(
						document.getElementsByName("orderCode")[0].value);
			}
			...
			// 3. callback
			Ajax.onreadystatechange = updateGoodsList;
			Ajax.open("post", "ASearchGoods", true);
			Ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8");
			param = "goodsCode="+goodsCode;
			if(orderCode != null){ param += "&orderCode="+orderCode}
			alert(param);
			Ajax.send(param);
		}
	}
	...
	function updateGoodsList() {
		...
		if(Ajax.readyState==4) {
			if(Ajax.status==200) {
				goodsList.innerHTML = Ajax.responseText;
			}else {
				alert(Ajax.status);
			}
		}
	...
	function sendGoCode() {
		if (window.event.keyCode == 13) {
			...
			form.setAttribute("action", "ASearchGoods");
				...
				<div class="pos__body">
					...
					<div class="pos__gocode">
						<span class="pos__gocode__title">상품코드</span><span
							...
							onKeyPress="ajaxSendGoCode()" /></span>
					</div>
